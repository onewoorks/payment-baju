          ...
          <div class="col-lg-6 mx-auto contentnya">
            ...
            <figure>
              <a
                ...
                href="https://toyyibpay.com/e/143251051126981"
                target="_blank"
              >
                <img
                  src="../assets/toyyibpay.png"
                  class="img-fluid"
                  alt="onewoorks-toyyibpay"
                  v-if="payment_confirm == true"
                />
              </a>
            </figure>
            ...
            <form @submit.prevent="how_much">
              <input
                ...
                placeholder="Please insert your jersey no!"
              />
              <div>
                <br />
                <transition name="slide-fade">
                  ...
                  <div v-if="duplicates != null">
                    ...
                    <ul v-for="(duplicate, index) in duplicates" :key="index" class>
                      <li
                        class="text-left"
                        @click="pick_me(duplicate)"
                      >{{ duplicate.nama.toUpperCase() }}</li>
                    </ul>
                  </div>
                </transition>

                <div
                  v-if="btn_check == null && payment_confirm == false "
                  @click="how_much"
                  class="btn btn-block btn-primary"
                >Search</div>
                <div class="btn-group btn-block" v-if="btn_check != null">
                  <div @click="search_again" class="d-none btn btn-outline-primary">Search Again</div>
                  <button
                    type="button"
  ...
  methods: {
    ...
    view_picker_info: function(info) {
      if (typeof info !== "undefined") {
        ...
        Axios.get(
          `https://api.onewoorks-solutions.com/payment_gateway/toyyibpay/find?refid=${this.detail.ref_key}`
        ).then(response => {
